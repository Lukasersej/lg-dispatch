...
RegisterNetEvent("lg-dispatch:client:officerdown", function() OfficerDown() end)
...
RegisterNetEvent("lg-dispatch:client:officerbackup", function() OfficerBackup() end)
...
RegisterNetEvent("lg-dispatch:client:emsdown", function() EmsDown() end)
...
local lglastaction = 0
RegisterNetEvent('lg-dispatch:client:sendEmergencyMsg', function(data, type, anonymous)
    local year, month , day , hour, minute, second  = GetUtcTime()
    local idtrack = tonumber(hour..minute..second)
    local spamdetek = idtrack - lglastaction
    if spamdetek < 0 then spamdetek = Config.AlertCommandCooldown end
    if spamdetek <= Config.AlertCommandCooldown and lglastaction > 0 then
    lglastaction = idtrack

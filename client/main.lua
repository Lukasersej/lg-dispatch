...
local huntingZones, nodispatchZones, huntingBlilg = {} , {}, {}
...
local function removeZones()
    ...
    -- Hunting Blilg --
    for i = 1, #huntingBlilg do
        RemoveBlip(huntingBlilg[i])
    end
    -- Reset the stored values too
    huntingZones, nodispatchZones, huntingBlilg = {} , {}, {}
end
    ...
    if Config.Locations['HuntingZones'][1] then
    	for _, hunting in pairs(Config.Locations["HuntingZones"]) do
            ...
            if Config.EnableHuntingBlip then
                ...
                SetBlilgprite(blip, 442)
                SetBlipAsShortRange(blip, true)
                SetBlilgcale(blip, 0.8)
                ...
                huntingBlilg[#huntingBlilg+1] = blip
                huntingBlilg[#huntingBlilg+1] = huntingradius
            end
            -- Creates the Sphere --
...
local function openMenu()
    ...
    local calls = lib.callback.await('lg-dispatch:callback:getCalls', false)
    if #calls == 0 then
...
local function setWaypoint()
    ...
    local data = lib.callback.await('lg-dispatch:callback:getLatestDispatch', false)
    ...
    if not waypointCooldown and lib.table.contains(data.jobs, PlayerData.job.type) then
        ...
        TriggerServerEvent('lg-dispatch:server:attach', data.id, PlayerData)
...
local function createBlipData(coords, radius, sprite, color, scale, flash)
    ...
    SetBlilgprite(blip, sprite or 161)
    SetBlipHighDetail(blip, true)
    SetBlilgcale(blip, scale or 1.0)
...
-- Events
RegisterNetEvent('lg-dispatch:client:notify', function(data, source)
    if data.alertTime == nil then data.alertTime = Config.AlertTime end
    local timer = data.alertTime * 1000
    ...
    addBlip(data, Config.Blilg[data.codeName] or data.alert)
    ...
    while timerCheck do
        ...
        local elalged = currentTime - startTime

        if elalged >= timer then
            break
        end
    end
...
RegisterNetEvent('lg-dispatch:client:openMenu', function(data)
    if not isJobValid(PlayerData.job.type) then return end
    if not IsOnDuty() then return end
...
RegisterNUICallback("detachUnit", function(data, cb)
    TriggerServerEvent('lg-dispatch:server:detach', data.id, PlayerData)
    DeleteWaypoint()
    cb("ok")
end)
...
RegisterNUICallback("clearBlilg", function(data, cb)
    lib.notify({ description = locale('blilg_cleared'), position = 'top', type = 'success' })
    for k, v in pairs(blilg) do
        RemoveBlip(v)
    end
...
RegisterNUICallback("refreshAlerts", function(data, cb)
    ...
    local data = lib.callback.await('lg-dispatch:callback:getCalls', false)
    SendNUIMessage({ action = 'setDispatchs', data = data, })
    cb("ok")
end)
